...
public class GuiDemo extends JFrame {
	...
	public GuiDemo() {
		...
		drawPanel.getTextItem().setText("Name: Jasper Albert Nri\n" + "University: University of the People.\n"
		...
		drawPanel.setBackgroundImage(Util.getImageResource("resources/images/new.jpeg"));
		...
		// add the method makeToolbar() to the NORTH of the existing content panel.
	...
	/*
	 * Creates and returns JToolBar object that duplicates the functionality of the background. This method adds background tools new pictures icons and saves them with actionlisteners.
	 */
	private JToolBar makeToolbar() {
		JToolBar tbar = new JToolBar("Background");
		JButton newPicture = tbar.add(newPictureAction);
		newPicture.setToolTipText("Clear this Image, return to default");
		JButton saveImage = tbar.add(saveImageAction);
		saveImage.setToolTipText("Save this Image");
	...
	private class ChooseBackgroundAction extends AbstractAction {
		...
		ChooseBackgroundAction(String text) {
			...
			if (text.equals("Color...")) {
				...
				// adding color to an image icon
				BufferedImage imageColorer = new BufferedImage(32, 32, BufferedImage.TYPE_INT_RGB);
				Graphics g = imageColorer.createGraphics();
				g.setColor(Color.RED);
				g.fillRect(0, 0, 20, 32);
				g.setColor(Color.GREEN);
				g.fillRect(10, 0, 15, 32);
				g.setColor(Color.BLUE);
				g.fillRect(20, 0, 15, 32);
				g.dispose();
				putValue(Action.SMALL_ICON, new ImageIcon(imageColorer));
			}
			// sets the action_icons/fileopen.png image as icon for the Custom...
