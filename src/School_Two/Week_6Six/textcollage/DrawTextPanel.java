...
* NEW FEATURES ADDED:
* 1. added support for undo 
* 2. each left click puts text with random background color, border, font, etc.
* 3. added support for saving and opening text files.
*/
/**
...
public class DrawTextPanel extends JPanel {
    ...
    private SimpleFileChooser fileChooser; // for letting the user select Õles
    private JMenuBar menuBar; // a menu bar with command that aàect this panel
    ...
    public void doMousePress(MouseEvent e) {
        ...
        s.setFont(new Font("Serif", fontStyle, (int) (Math.random() * 15 + 8)));
        // create diàerent types of magniÕcation
        s.setMagnification((int) (Math.random() * 5 + 1));
        ...
        s.setTextTransparency(Math.random() * 0.35);
        ...
        s.setBackgroundTransparency(Math.random() * 0.80 + 0.20);
        if (theStrings == null)
    ...
    /**
     * Returns a menu bar containing commands that aàect this panel. The menu
    ...
    private void doMenuCommand(String command) {
        if (command.equals("Save...")) { // save all the string info to a Õle
            saveTextFile();
        } else if (command.equals("Open...")) { // read a previously saved Õle, and reconstruct the list of strings
            openTextFile();
            canvas.repaint(); // (you'll need this to make the new list of strings take eàect)
            ...
            try {
                ...
                // There is a convenient method for writing a BufferedImage to a Õle.
                ...
                boolean ok = ImageIO.write(image, "PNG", imageFile); // write to the Õle
                if (ok == false)
    ...
    /**
     * MY EXTRA FEATURES SECTION
     */
    /**
     * THE UNDO COMMAND.
     * Ctrl-Z will invoke this method.
     */
    ...
    private void saveTextFile() {
        ...
        FileNotFoundException e) {
            JOptionPane.showMessageDialog(this, "Can't write to the Õle \"" + saveAs + "\".");
    ...
    /**
     * OPEN FILE
     * Read a previously saved file's background color, and reconstruct the list of
     * strings
     */
    private void openTextFile() {
        File openTextFile = fileChooser.getInputFile(this, "Open Saved File");
        try {
            Scanner in = new Scanner(openTextFile);
            if (!in.nextLine().equals("New text collage Õle")) {
                JOptionPane.showMessageDialog(this, "Not a valid Õle \"" + openTextFile + "\".");
                return;
            }
        ...
        FileNotFoundException e) {
            JOptionPane.showMessageDialog(this, "Can't read the Õle \"" + openTextFile + "\".");
            System.out.println("Error message: " + e);
        }
    }
}